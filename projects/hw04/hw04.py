...
def taxicab(a, b):
    ...
    9
    """
    a_st, a_ave = street(a), avenue(a)
    b_st, b_ave = street(b), avenue(b)
    return abs(a_st-b_st) + abs(a_ave-b_ave)
...
def balanced(m):
    ...
    if balanced(end(left_side)) == True:
        left_weight = length(left_side) * total_weight(end(left_side))
    else:
        return False
    if balanced(end(right_side)) == True:
    ...
    #     left_weight = length(left_side) * total_weight(end(left_side))
    # else:
    #     flag_left = balanced(end(left_side))
    #     if flag_left == True:
    #         left_weight = length(left_side) * total_weight(end(left_side))
    #     else:
    #         return False
    ...
    #     right_weight = length(right_side) * total_weight(end(right_side))
    # else:
    #     flag_right = balanced(end(right_side))
    #     if flag_right == True:
    #         right_weight = length(right_side) * total_weight(end(right_side))
    #     else:
    #         return False
...
def totals_tree(m):
    ...
    return [label, left_tree, right_tree]
...
def replace_leaf(t, old, new):
    ...
    if not is_leaf(t):
        for branch in branches(t):
            new_branch = replace_leaf(branch, old, new)
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
    tip = withdraw(0, old_password)
    if type(tip) == int:   #密码正确
        def inner_withdraw(amount, inner_password):
            if inner_password == new_password:
                return withdraw(amount, old_password)
            else:
                return withdraw(amount, inner_password)
...
def quadratic(x, a, b, c):
    ...
    """
    extreme_point = -b/(2*a)
    min_or_max = a * extreme_point**2 + b * extreme_point + c
    ...
    if a != 0:
        if extreme_point > lower_bound(x) and extreme_point < upper_bound(x):
            return interval(min(left, right, min_or_max), max(left, right, min_or_max))
        else:
            return interval(min(left, right), max(left, right))
    else:
        return add_interval(interval(c, c) , mul_interval(interval(b, b), x))
