...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        f_name = target.first
        f_function = do_lambda_form(Pair(target.rest, expressions.rest), env)
        # print(f_function)
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return scheme_truep(scheme_eval(expressions, env, True))
    while expressions != nil:
        tail = expressions.rest is nil
        first_val = scheme_eval(expressions.first, env, tail)
        if scheme_falsep(first_val):
            return first_val
...
def do_or_form(expressions, env):
    ...
    # BEGIN SOLUTION "PROBLEM 12"
    if expressions == nil:
        return scheme_falsep(scheme_eval(expressions, env))
    while expressions != nil:
        tail = expressions.rest is nil
        first_val = scheme_eval(expressions.first, env, tail)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        f_function = MacroProcedure(expressions.first, expressions.rest, env)
        env.define(f_name, f_function)
        return f_name
    else:
        bad_target = target.first if isinstance(target, Pair) else target
